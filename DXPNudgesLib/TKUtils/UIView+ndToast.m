...
// general appearance
static const CGFloat CSToastMaxWidth            = 0.6;      // 80% of parent view width  modify by libiao 2021-04-28
static const CGFloat CSToastMaxHeight           = 0.8;      // 80% of parent view height
static const CGFloat CSToastHorizontalPadding   = 10.0;
static const CGFloat CSToastVerticalPadding     = 10.0;
static const CGFloat CSToastCornerRadius        = 10.0;
static const CGFloat CSToastOpacity             = 0.8;      // modify by libiao 2021-04-28
...
// positions
NSString * const CSNdToastPositionTop             = @"top";
NSString * const CSNdToastPositionCenter          = @"center";
NSString * const CSNdToastPositionBottom          = @"bottom";

@interface UIView (ndToastPrivate)
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image isWrapperViewApha:(BOOL)isWrapperViewApha;
- (CGSize)sizeForString:(NSString *)string font:(UIFont *)font constrainedToSize:(CGSize)constrainedSize lineBreakMode:(NSLineBreakMode)lineBreakMode;

@end


@implementation UIView (ndToast)
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position isWrapperViewApha:(BOOL)isWrapperViewApha {
    UIView *toast = [self viewForMessage:message title:nil image:nil isWrapperViewApha:isWrapperViewApha];
    [self showToast:toast duration:duration position:position];
}
    
- (void)showCenterToast:(NSString *)message isShadowOpacity:(BOOL)isShadowOpacity{
//    UIView *toast = [self viewForMessage:message title:nil image:nil isShadowOpacity:isShadowOpacity];
//    [self showToast:toast duration:1.f position:CSNdToastPositionCenter];
}
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:CSNdToastPositionTop] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width/2, (toast.frame.size.height / 2) + CSToastVerticalPadding);
        } else if([point caseInsensitiveCompare:CSNdToastPositionCenter] == NSOrderedSame) {
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    return [self viewForMessage:message title:title image:image isWrapperViewApha:YES];
}
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image isWrapperViewApha:(BOOL)isWrapperViewApha{
    // sanity
    ...
    wrapperView.backgroundColor = isWrapperViewApha?[UIColorFromRGB_Nd(0x3B3B3B) colorWithAlphaComponent:CSToastShadowOpacity]:[UIColor blackColor];
    ...
    if (message != nil) {
        ...
        messageLabel.textAlignment = NSTextAlignmentCenter;  // add by libiao
