        ...
        if (data) {
            ...
            @try {
                if ([dic isKindOfClass:[NSDictionary class]]) {
                    NSLog(@"DXPNudges Log:===   ResponseUrl======Fail===\nStatusCode:%ld\n==RequestUrl:\n%@\n==HeaderFields:\n%@\n==Error!ResponseBody:\n==ITRACING_TRACE_ID:\n%@\n%@\n",self.httpURLResponse.statusCode,[self.httpURLResponse.URL absoluteString], self.httpURLResponse.allHeaderFields,[ self.httpURLResponse.allHeaderFields objectForKey:@"ITRACING_TRACE_ID"],[[dic jsonPrettyStringEncoded] jsonStrHandle]);
                } else {
                    NSLog(@"DXPNudges Log:===   ResponseUrl======Fail===StatusCode:%ld==RequestUrl:%@==HeaderFields:%@==ResponseBody:%@",self.httpURLResponse.statusCode,[self.httpURLResponse.URL absoluteString], self.httpURLResponse.allHeaderFields,dic);
                }
            } @catch (NSException *exception) {
            } @finally {
            }
            HJLog(@"DXPNudges Log:===   ========响应体打印结束========");
    ...
    if (self.responseData.length > 0) {
        ...
        @try {
            if ([res isKindOfClass:[NSDictionary class]]) {
                NSLog(@"DXPNudges Log:===  ResponseUrl======Success\n===StatusCode:%ld\n==RequestUrl:%@==\nHeaderFields:%@==\n%@\nResponseBody:\n%@\n",self.httpURLResponse.statusCode,[self.httpURLResponse.URL absoluteString], [[self.httpURLResponse.allHeaderFields jsonPrettyStringEncoded] jsonStrHandle],[self.httpURLResponse.URL absoluteString],[[res jsonPrettyStringEncoded] jsonStrHandle]);
            } else {
                NSLog(@"DXPNudges Log:===  ResponseUrl======Success===StatusCode:%ld==RequestUrl:%@==HeaderFields:%@==ResponseBody:%@",self.httpURLResponse.statusCode,[self.httpURLResponse.URL absoluteString], self.httpURLResponse.allHeaderFields,res);
            }
        } @catch (NSException *exception) {
        }
        HJLog(@"========响应体打印结束========");
