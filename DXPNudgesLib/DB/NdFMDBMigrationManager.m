...
// Public Constants
NSString *const NdFMDBMigrationManagerErrorDomain = @"com.layer.NdFMDBMigrationManager.errors";
NSString *const NdFMDBMigrationManagerProgressVersionUserInfoKey = @"version";
NSString *const NdFMDBMigrationManagerProgressMigrationUserInfoKey = @"migration";

// Private Constants
static NSString *const FMDBMigrationFilenameRegexString = @"^(\\d+)_?((?<=_)[\\w\\s-]+)?(?<!_)\\.sql$";

BOOL NdFMDBIsMigrationAtPath(NSString *path)
{
...
@interface NdFMDBMigrationManager ()
...
@implementation NdFMDBMigrationManager
...
- (void)addMigration:(id<NdFMDBMigrating>)migration
{
...
- (void)addMigrations:(NSArray *)migrations
{
    ...
    for (id<NSObject> migration in migrations) {
        if (![migration conformsToProtocol:@protocol(NdFMDBMigrating)]) {
...
- (NSArray *)migrations
{
    ...
    for (NSString *path in migrationPaths) {
        ...
        if ([migrationRegex rangeOfFirstMatchInString:filename options:0 range:NSMakeRange(0, [filename length])].location != NSNotFound) {
            NdFMDBFileMigration *migration = [NdFMDBFileMigration migrationWithPath:path];
    ...
    if (self.dynamicMigrationsEnabled) {
        NSArray *conformingClasses = FMDBClassesConformingToProtocol(@protocol(NdFMDBMigrating));
        for (Class migrationClass in conformingClasses) {
            if ([migrationClass isSubclassOfClass:[NdFMDBFileMigration class]]) continue;
            id<NdFMDBMigrating> migration = [migrationClass new];
            [migrations addObject:migration];
        }
    }
...
- (id<NdFMDBMigrating>)migrationForVersion:(uint64_t)version
{
    for (id<NdFMDBMigrating>migration in [self migrations]) {
        if (migration.version == version) return migration;
    }
    return nil;
}

- (id<NdFMDBMigrating>)migrationForName:(NSString *)name
{
    for (id<NdFMDBMigrating>migration in [self migrations]) {
...
- (BOOL)migrateDatabaseToVersion:(uint64_t)version progress:(void (^)(NSProgress *progress))progressBlock error:(NSError **)error
{
    ...
    for (NSNumber *migrationVersionNumber in pendingVersions) {
        ...
        id<NdFMDBMigrating> migration = [self migrationForVersion:migrationVersion];
        ...
        if (progressBlock) {
            [progress setUserInfoObject:@(migrationVersion) forKey:NdFMDBMigrationManagerProgressVersionUserInfoKey];
            [progress setUserInfoObject:migration forKey:NdFMDBMigrationManagerProgressMigrationUserInfoKey];
            progressBlock(progress);
            if (progress.cancelled) {
                success = NO;
                
                NSDictionary *userInfo = @{ NSLocalizedDescriptionKey: @"Migration was halted due to cancellation." };
                if (error) *error = [NSError errorWithDomain:NdFMDBMigrationManagerErrorDomain code:FMDBMigrationManagerErrorMigrationCancelled userInfo:userInfo];
                [self.database rollback];
                break;
            }
        }
...
static BOOL FMDBMigrationScanMetadataFromPath(NSString *path, uint64_t *version, NSString **name)
{
    ...
    if (!regex) {
        NSLog(@"DXPNudges Log:===  [FMDBMigration] Failed constructing regex: %@", error);
        return NO;
    }
...
@interface NdFMDBFileMigration ()
@property (nonatomic, readwrite) NSString *name;
@property (nonatomic, readwrite) uint64_t version;
@end

@implementation NdFMDBFileMigration
